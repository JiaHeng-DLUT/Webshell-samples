...
function get_thumb($addr,$title='',$div='') {
     if(strpos($addr,'://')>1) $imgsmall = $addr;
      ...
      if(!is_file($imgsmallroot))  $imgsmall = DEFAULTIMG;
     }
...
function get_img($addr,$title='',$div='') {
	if(strpos($addr,'://')>1) $imgsmall = $addr;
		...
		$imgsmall=UPLOADPATHIMAGE.$addr;
		$imgsmallroot=UPLOADROOTIMAGE.$addr;
       if(!is_file($imgsmallroot))  $imgsmall = DEFAULTIMG;		
	}
...
/*************************/ 
function alias($pid) { //type=page,cate,detail...no csub
global $andlangbh;
 $sql = "SELECT name from ".TABLE_ALIAS."  where  pid='$pid'   $andlangbh  order by id limit 1";
//	 echo $sql;exit;
	  if(getnum($sql)>0){
	      $row=getrow($sql);
				//echo '===================='.$row['name'];
...
//----------------------
 function get_url($arr){  
     $alias_jump = $arr['alias_jump'];
     $pidname = $arr['pidname'];$tid = $arr['id'];

     $pidname4 = substr($pidname,0,4); $pidname3 = substr($pidname,0,3);
	 if($pidname3=='tag')  $pidname4 = 'tagg';
	 
     if($pidname4=='node') $type='detail-';
     else if($pidname4=='page') $type='page-';
	 else if($pidname4=='tagg') $type='tag-'; 
     else  $type='category-';

     if($alias_jump<>'') $rev = $alias_jump; 
     else{
          $alias = alias($pidname);
         if($alias<>'') $rev = $alias.'.html';
          else  $rev = $type.$tid.'.html';
     }
      return $rev;

 }
//------------
 function get_link($url,$name,$css=''){  
    $class = ' class="'.$css.'" ';
    $cssv = $css==''?'':$class; 
      return '<a  '.linkhref($url).$cssv.'>'.$name.'</a>';
