...
$sqlsub = "SELECT  *  from " . TABLE_MUSIC . "  where  pidname='$pidname'   $andlangbh order by id limit 1";
//echo $sqlsub;exit;
    if(getnum($sqlsub)>0){
      ...
      echo 'music not exist...';
      exit;
      }
  }

$jumpv='mod_mainmusic.php?lang='.LANG.'&pid='.$pid;
$jumpvnopid='mod_mainmusic.php?lang='.LANG;//when back from insert
$jumpvsubgl='mod_music.php?lang='.LANG;
...
if($act == "del_block")
{
  ...
  $ifdel1 =  ifcandel_field(TABLE_MUSIC,'pid',$pidname,'equal','出错，音乐里还有mp3，请先删除！',$jumpv);


   ifsuredel_field(TABLE_MUSIC,'pidname',$pidname,'eq',$jumpv);
...
$title2='';
if($act<>'list'){
  $title1='<a href="'.$jumpv.'" style="font-size:18px">音乐区块管理</a>';
...
if($act=='' || $act=='list')  require_once HERE_ROOT.'mod_music/tpl_mainmusic_list.php';
else  require_once HERE_ROOT.'mod_music/tpl_mainmusic_addedit.php';
?>
<div class="c"></div>
