...
if($act=="update"){
   ...
   $kv_v = '';
	if(!empty($imgname)){
		$sql = "SELECT kvsm from ".TABLE_MUSIC." where pid='$pid' and type='$type' and id='$tid' $andlangbh   limit 1";
		$rowss = getrow($sql);
 ...
 $desp=zbdesp_onlyinsert($_POST['desp']); 
			...
			$ss = "update ".TABLE_MUSIC." set title='$abc1'$kv_v,desp='$desp' where pid='$pid' and type='$type' and id='$tid' $andlangbh  limit 1";
			 //echo $ss;exit;
			iquery($ss); 	
  ...
  $sql="select * from  ".TABLE_MUSIC."  where  pid='$pid' and type='$type' and id='$tid' $andlangbh order by id limit 1";
  //echo $sql;exit;
   $row = getrow($sql);
...
<?php 
 require_once HERE_ROOT.'mod_music/tpl_album_edit_sidebar.php';
 ?>
</div><!--end sidebar-->
