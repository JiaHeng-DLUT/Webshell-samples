...
<?php
 ...
 //if($key == "") $keyV="" ;
  ...
  $sql = "SELECT * from ".TABLE_MUSIC." where   $noandlangbh  and pid='$pid' order by pos desc,id desc"; //pos desc,id desc
    // echo $sql;
  $num_rows = getnum($sql);
     if($num_rows>0){
       $maxline = 300;
...
//----------------------
 ...
 $num = num_subnode(TABLE_MUSIC,'pid',$pidname);
$edit_text= "<a class='but1'  href='$jumpv&pidname=$pidname&act=edit'><i class='fa fa-pencil-square-o'></i> 修改</a>";
...
$gl= "<a class='but1'  href='$jumpvsubgl&pid=$pidname'><i class='fa fa-folder'></i> 管理($num)</a><br />";
