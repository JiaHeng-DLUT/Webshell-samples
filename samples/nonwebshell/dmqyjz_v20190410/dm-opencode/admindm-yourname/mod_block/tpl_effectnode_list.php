...
<?php
   ...
   $sql = "SELECT * from ".TABLE_NODE." where pid='$pid' and modtype='blockdh' $andlangbh order by pos desc,id "; 
   if(getnum($sql)==0){ 
          echo '<br><br>没有找到相关内容。'; 
}
    else {
...
$del_text= " <a href=javascript:del('delnode','$pidname','$jumpvpid','$jsname')  class=but2><i class='fa fa-trash-o'></i> 删除</a>";
 ...
 <?php echo $name;?> <br />
  ...
  if($linkdhtitle<>'') echo '链接字样：'.$linkdhtitle.'<br />';
  if($linkdhurl<>'') echo '链接：'.$linkdhurl.'<br />';

   if($titlebz1<>'') echo '备注1：'.$titlebz1.'<br />';
  if($titlebz2>'') echo '备注2：'.$titlebz2.'<br />';
  if($titlebz3<>'') echo '备注3：'.$titlebz3.'<br />';
  if($despjj<>'') echo '简介：'.substr($despjj,0,200).'<br />';
...
if($kvsm2<>'')   echo  p2030_imgyt($kvsm2,'y','n');
//echo   get_img($kvsm2,'','div');
 
?>
</div>
<p><a class="needpopup but4 pad8lr" style="width:auto" href="../mod_module/mod_uploadkvsm.php?lang=<?php echo LANG?>&pidname=<?php echo $pidname?>&type=nodekvsm2">修改缩略图2</a></p>
</td>
