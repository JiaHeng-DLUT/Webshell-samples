        ...
        <div class="layui-card-header layuiadmin-card-header-auto layui-form">
            ...
            <div>
                <div class="layui-btn-group">
                    <a class="layui-btn layui-btn-sm" href="<?php echo e(route('admin.virtual.comment.create')); ?>">+虚拟评论</a>
    ...
    <script>
        layui.use(['layer','table','form','laydate'],function () {
            ...
            var dataTable = table.render({
                elem: '#dataTable'
                ,url: "<?php echo e(route('admin.comment.data')); ?>" //数据接口
            ...
            table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'del'){
                    layer.confirm('确认删除吗？', function(index){
                        $.post("<?php echo e(route('admin.comment.destroy')); ?>",{_method:'delete',id:data.id},function (result) {
                            if (result.code==0){
            ...
            $("#listAuditPass").click(function () {
                ...
                if(ids.length>0)
                {
                    layer.confirm('确认批量审核吗？', function(index){
                        $.post("<?php echo e(route('admin.comment.auditPass')); ?>",{ids:ids},function (result) {
                            if (result.code==0){
                                dataTable.reload()
                                layer.close(index);
                                layer.msg(result.info,{icon:6})
            ...
            $("#listNotShown").click(function () {
                ...
                if(ids.length>0)
                {
                    layer.confirm('确认批量不予展示吗？', function(index){
                        $.post("<?php echo e(route('admin.comment.notAuditPass')); ?>",{ids:ids},function (result) {
                            if (result.code==0){
                                dataTable.reload()
                                layer.close(index);
                                layer.msg(result.info,{icon:6})
            ...
            $("#listDelete").click(function () {
                ...
                if (ids.length>0){
                    layer.confirm('确认删除吗？', function(index){
                        $.post("<?php echo e(route('admin.comment.auditDestroy')); ?>",{ids:ids},function (result) {
                            if (result.code==0){
            ...
            $("#listExport").click(function () {
                ...
                var model_name = $("#model_name").val();
                var comment_type = $("#comment_type").val();
                var u = '<?php echo e(route('admin.comment.export')); ?>'
