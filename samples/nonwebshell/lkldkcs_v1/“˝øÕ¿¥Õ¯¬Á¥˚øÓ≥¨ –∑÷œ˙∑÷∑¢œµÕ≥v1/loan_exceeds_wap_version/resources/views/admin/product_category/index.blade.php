        ...
        <div class="layui-card-body">
            ...
            <div class="layui-btn-group layui-inline" style="padding-bottom: 10px">
                @can('product.category.create')
                    <a class="layui-btn layui-btn-sm" href="{{ route('admin.productCategory.create') }}">添 加</a>
                @endcan
    ...
    <script>
        layui.use(['layer','table','form'],function () {
            ...
            var dataTable = table.render({
                elem: '#dataTable'
                ,url: "{{ route('admin.productCategory.data') }}" //数据接口
                ,page: true //开启分页
                ,autoSort: false
                ,cols: [[ //表头
            ...
            table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'del'){
                    layer.confirm('请确认需要删除'+data.name+'类目？', function(index){
                        $.post("{{ route('admin.productCategory.destroy') }}",{_method:'delete',ids:[data.id]},function (result) {
                            if (result.code===0){
                                delReload(dataTable)
            ...
            $("#listDelete").click(function () {
                ...
                if (ids.length>0){
                    layer.confirm('确认删除吗？', function(index){
                        $.post("{{ route('admin.productCategory.destroy') }}",{_method:'delete',ids:ids},function (result) {
                            if (result.code===0){
