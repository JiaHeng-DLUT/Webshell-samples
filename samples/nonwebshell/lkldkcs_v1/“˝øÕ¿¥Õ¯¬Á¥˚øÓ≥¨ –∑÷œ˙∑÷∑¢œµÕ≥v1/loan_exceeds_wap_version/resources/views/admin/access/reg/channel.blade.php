...
@section('content')
    @php
        $endDate=\Carbon\Carbon::parse('-1 day')->toDateString();
        if(time()<strtotime(date('Y-m-d 10:00',time()))){
            $endDate=\Carbon\Carbon::parse('-2 day')->toDateString();
        }
    @endphp
    ...
    <script>
        layui.use(['layer','table','form','laydate'],function () {
            ...
            var dataTable = table.render({
                elem: '#dataTable'
                ,url: "{{ route('admin.access.reg.channelData') }}" //数据接口
                ,page: true //开启分页
                ,autoSort: false
                ,cols: [[ //表头
