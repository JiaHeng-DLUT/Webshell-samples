...
@section('content')
    <div class="layui-card">
        <div class="layui-card-header">PUSH消息@if(!isset($info))添加@else编辑@endif</div>
        <div class="layui-card-body" style="padding: 15px;">
            <form class="layui-form" method="post">
                {{ csrf_field() }}
                <input type="hidden" name="type" value="push">
                <div class="layui-form-item">
                    <label class="layui-form-label"><strong class="item-required">*</strong>push名称</label>
                    <div class="layui-input-block">
                        <input type="text" name="title" lay-verify="name" autocomplete="off" placeholder="请输入2-20个字push名称" class="layui-input" value="{{ $info->title??'' }}" maxlength="20">
                    </div>
                </div>
                <div class="layui-form-item layui-form-text">
                    <label class="layui-form-label"><strong class="item-required">*</strong>推送内容</label>
                    <div class="layui-input-block">
                        <textarea name="content" placeholder="请输入50个字以内的推送内容" class="layui-textarea" lay-verify="introduce" maxlength="50"  id="replyContent" >{{ $info->content??'' }}</textarea>
                        <span class="count" ><span id="count">@if(isset($info)) {{strlen($info->content)}} @else 0 @endif</span>/50</span>
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label"><strong class="item-required">*</strong>链接页面</label>
                    <div class="layui-input-block">
                        <select name="redirect_model" lay-filter="redirect_model" lay-verify="required">
                            <option value="" @if(!isset($info))  selected @endif>请选择</option>
                            <option value="AppIndex" @if(isset($info) && $info->redirect_model == 'AppIndex') selected  @endif>APP首页</option>
                            <option value="AppProduct" @if(isset($info) && $info->redirect_model == 'AppProduct') selected  @endif>APP产品页</option>
                            <option value="AppUserCenter"  @if(isset($info) && $info->redirect_model == 'AppUserCenter') selected  @endif>APP个人中心</option>
                            <option value="CustomLink" @if(isset($info) && $info->redirect_model == 'CustomLink') selected  @endif >自定义链接</option>
                        </select>
                    </div>
                </div>
                <div class="layui-form-item custom-link"  style="@if(isset($info) && $info->redirect_model == 'CustomLink') display: block; @else display: none;  @endif ">
                    <label class="layui-form-label">自定义链接</label>
                    <div class="layui-input-block">
                        <input type="text" name="redirect_model_detail" lay-verify="" autocomplete="off" placeholder="请输入自定义链接" class="layui-input" value="{{ $info->redirect_model_detail??'' }}" maxlength="20" >
                    </div>
                </div>
                <div class="layui-form-item">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        ...
                        <input type="radio" name="user_type" lay-filter="user_type" value="3" title="指定用户" @if(isset($info) && $info->send_object == 3 ) checked @endif>
                        <div class="layui-unselect layui-form-radio"><i class="layui-anim layui-icon"></i>
                            <div>指定注册用户</div>
                        </div>
                    </div>
                ...
                <div class="layui-form-item">
                    <label for="" class="layui-form-label"><strong class="item-required">*</strong>推送应用</label>
                    <div class="layui-input-block">
                        {{--<select xm-select="application" lay-ignore="" _name="application" _lay-verify="select14" style="display: inline-block;">--}}
                        <select xm-select="application" lay-ignore="" _name="application" _lay-verify="required" style="display: inline-block;">
                            @if($models)
                                @foreach($apps as $item)
                                    <option value="{{$item->id}}" @if(isset($info) && in_array($item->id,$info['application'])) selected @endif>{{$item->display_name}}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
