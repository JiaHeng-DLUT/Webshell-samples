...
//        ]);
     ...
     * 接收1.2用户同步数据
     */
    public function getOldUserData(Request $request)
    {
//        Log::info('--request--'.json_encode($request->all()));
        $exist = Member::where(['phone'=>$request->mobile])->first();
        if(!count($exist)){
            if($request->crypto) {
            ...
            $data['app_id'] = 0;
            $data['nick'] = substr($request->mobile, 0, 3) . '****' . substr($request->mobile, 7);
            $data['platform_register'] = $request->platform??'';
            $data['platform_login'] = $request->platform??'';
            $data['channel_code'] = $request->channel;
            $data['old_page_id'] = $request->page_id;
            $data['page_id'] = 0;
            $data['reduce_type'] = 'register';
            $data['reduce_rate'] = 100;
            $data['identifier_register'] = $request->identifier;
            $data['identifier_login'] = $request->identifier;
            $data['identifier_push'] = '';
            $data['register_ip'] = $request->ip;
            $data['last_login_ip'] = '';
            $data['last_login_at'] = date('Y-m-d H:i:s',$request->created_at);
            $data['created_at'] = date('Y-m-d H:i:s',$request->created_at);
            $insert = Member::create($data);
            if ($insert) {
                $insert->created_at = $data['created_at'];
            } else {
                $this->set('type', $this->serviceError);
                $this->set('info', '服务器错误');
            }
        }else{
            $this->set('info', '用户已存在');
