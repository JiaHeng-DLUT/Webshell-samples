	...
	*/
	...
	MagnifierF.prototype = {
		init: function(id){
			...
			this.mImgSrc =  this.magImg.getAttribute('bigimg');
		...
		specFn: function(){
			...
			function TabFn(event){   
				...
				if (target.nodeName != "UL") { 
					...
					_oISrc_bigimg = _oSpImg.getAttribute('bigimg'); 
					
					_is.magImg.setAttribute('src',_oISrc);
					_is.magImg.setAttribute('jqimg',_oISrc_bigimg);
					//_is.mImgSrc =  _oISrc.slice(0,-4);
					//_is.mImgSrc =  _oSpImg.getAttribute('bigimg');
				}
