...
class Index extends Controller
{
    ...
    private function step4()
    {
        if ($this->request->isPost()) {
            if (!is_writable(QL_ROOT.'config/database.php')) {
            ...
            $cover = $data['cover'];
            unset($data['cover']);
            $config = include QL_ROOT.'config/database.php';
            foreach ($data as $k => $v) {
    ...
    private function step5()
    {
        ...
        $rule = [
            'username|管理员账号' => 'require|length:3,15|alphaNum',
       ...
       $sql_file = APP_PATH.'install/sql/install.sql';
        if (file_exists($sql_file)) {
            ...
            $sql_list = parse_sql($sql, ['ql_' => $config['prefix']] ,0);
            if ($sql_list) {
        ...
        $array = [
                ...
                'username'      => $username,
                'passsalt'      => $passsalt,
                'password'      => password_key($username,$password,$passsalt),
        ];
     
        Db::name("admin")->insert($array);
        ...
        Db::name("admin")->where('uid',1)->update($array);

        // 获取站点根目录
        $root_dir  = request()->baseFile();
        $root_dir  = preg_replace(['/index.php$/'], [''], $root_dir);
        return $this->success('系统安装成功，欢迎您使用奇乐CMS安装系统', $root_dir.'?step=6');
    }

  private function step6()
  { 
      $str = '欢迎使用奇乐CMS安装系统,祝您使用愉快[官网：http://www.qilecms.com |论坛： http://bbs.qilecms.com]';
      write_file(QL_ROOT.'data/install.lock',$str);
...
INFO;
        write_file(QL_ROOT.'config/database.php', $code);
        // 判断写入是否成功
        $config = include QL_ROOT.'config/database.php';
