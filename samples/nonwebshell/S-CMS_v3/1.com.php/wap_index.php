...
switch ($U_type) {
    case "index":
        $page_info = e(j(CreateIndex(c(i(h($style, 1)),$U_type,true))));
        break;

    case "contact":
        $page_info = e(j(CreateContact(c(i(h($style, 1)),$U_type,true))));
        break;

    case "guestbook":
        $page_info = e(j(CreateGuestbook(c(i(h($style, 1)),$U_type,true))));
        break;
    ...
    case "text":
        if (getrs("select * from SL_text where T_del=0 and T_id=" . intval($S_id), "T_title") == "") {
            ...
            $page_info = e(j(CreateText(c(i(h($style, $S_id)),$U_type,true) , $S_id)));
        }
        break;
    ...
    case "form":
        if (getrs("select * from SL_form where F_del=0 and F_id=" . intval($S_id), "F_title") == "") {
            ...
            $page_info = e(j(CreateForm(c(i(h($style, $S_id)) ,$U_type,true), $S_id)));
        }
        break;
        ...
        if (is_numeric($S_id)) {
            if (getrs("select * from SL_nsort where  S_del=0 and S_id=" . intval($S_id), "S_title") == "" && $S_id <> 0) {
                ...
                if(getrs("select * from SL_nsort where S_del=0 and S_id=" . intval($S_id), "S_show")==0 && $S_id!="0"){
                    ...
                    $page_info = e(j(CreateNewsList(c(i(h($style, $S_id)) ,$U_type,true), $S_id, $S_page)));
                }
            }
        } else {
            $page_info = e(j(CreateNewsList(c(i(h($style, $S_id)) ,$U_type,true), $S_id, $S_page)));
    ...
    case "newsinfo":
        if (getrs("select * from SL_news where N_del=0 and N_id=" . intval($S_id), "N_title") == "") {
            ...
            $page_info = e(j(CreateNewsInfo(c(i(h($style, $S_id)) ,$U_type,true), $S_id)));
        }
        break;
    ...
    case "product":
        if (is_numeric($S_id)) {
            if (getrs("select * from SL_psort where S_del=0 and S_id=" . intval($S_id), "S_title") == "" && $S_id > 0) {
                ...
                $page_info = e(j(CreateProductList(c(i(h($style, $S_id)) ,$U_type,true), $S_id, $S_page)));
            }
        } else {
            $page_info = e(j(CreateProductList(c(i(h($style, $S_id)),$U_type,true) , $S_id, $S_page)));
        }
    ...
    case "productinfo":
        if (getrs("select * from SL_product where P_del=0 and P_id=" . intval($S_id), "P_title") == "") {
            ...
            $page_info = e(j(CreateProductInfo(c(i(h($style, $S_id)) ,$U_type,true), $S_id)));
        }
        break;
