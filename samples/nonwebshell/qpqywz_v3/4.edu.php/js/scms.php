...
if(md5($_GET["id"].$_GET["timestamp"]."unlike")==$_GET["key"]){
	if(time()*1000-$_GET["timestamp"]<1000){
		switch($type){
			case "t":
			mysqli_query($conn,"update SL_text set T_like=T_like-1 where T_like>0 and T_id=".$typeid);
			break;
			case "n":
			mysqli_query($conn,"update SL_news set N_like=N_like-1 where N_like>0 and N_id=".$typeid);
			break;
			case "p":
			mysqli_query($conn,"update SL_product set P_like=P_like-1 where P_like>0 and P_id=".$typeid);
			break;
		}
		echo "success";
	}else{
